...
<AccordionGroup>
  ...
  <Accordion title="Monitor failures">
    Subscribe a `webhook.delivery_failed` event to your alerting; or watch the
    `last_delivery_status` field on the endpoint resource. A `failed` state for
    more than an hour is your cue to investigate.
  </Accordion>
