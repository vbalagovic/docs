...
<Steps>
  ...
  <Step title="Install the SDK (or use cURL)">
    The TypeScript SDK is the fastest path on Node and the browser.
    ...
    ```bash pnpm
    pnpm add @neuraldraft/sdk
    ```

    ```bash bun
    bun add @neuraldraft/sdk
    ```

    ```bash yarn
    yarn add @neuraldraft/sdk
    ```

    </CodeGroup>

    Prefer cURL or another language? Skip the install — every endpoint is a
    plain HTTP call.

    <Tip>
      Python and PHP SDKs ship in the next release. Until then, see the
      [snippets](/snippets/vanilla) for hand-rolled clients in either language.
    </Tip>
  </Step>
