...
<Steps>
  ...
  <Step title="Pick your client">
    Every endpoint is a plain HTTP call — `fetch`, `axios`, `requests`, `cURL`,
    Postman, anything. There's no required SDK.

    <Tip>
      An official `@neuraldraft/sdk` (TypeScript), `neuraldraft` (Python) and
      `neuraldraft/sdk` (PHP) are on the roadmap. Until they land, the snippets
      below use built-in `fetch` — copy/paste straight into your codebase.
    </Tip>
  </Step>
  ...
  <Step title="Make your first call">
    ...
    ```ts Node (fetch)
    const apiKey = process.env.NEURAL_DRAFT_API_KEY!;
    const base = "https://api.neuraldraft.io/v1";
    const headers = {
    ...
    // Verify the key
    const me = await fetch(`${base}/projects/me`, { headers }).then(r => r.json());
    console.log(`Hello, ${me.data.name}`);

    // Create a draft blog post
    const draft = await fetch(`${base}/blog-posts`, {
      method: "POST",
      headers,
      body: JSON.stringify({
        title: "Five things we shipped this week",
    ...
    console.log(`Draft created: /blog/${draft.data.slug}`);
    ```
