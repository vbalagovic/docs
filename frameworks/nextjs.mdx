...
if (!process.env.NEURALDRAFT_API_KEY) {
  throw new Error("Missing NEURALDRAFT_API_KEY in env.");
}

export const neuralDraft = new NeuralDraftClient({
...
export function verifyNeuralDraftSignature(
  body: string,
  header: string,
  secret = process.env.NEURALDRAFT_WEBHOOK_SECRET ?? "",
...
export default async function PostPage({
  ...
  const post = await neuralDraft.blogPosts.get(slug, { lang: "en" }).catch(() => null);
  if (!post) notFound();
...
export async function ProductCard({ slug }: { slug: string }) {
  const product = await neuralDraft.products.get(slug);
...
> _"Add a `/about` page in the App Router. Use the Neural Draft SDK
> (`neuralDraft` from `lib/neural-draft.ts`) to fetch the matching `Page`
> via `pages.get('about')`, render `meta_title` / `meta_description` /
> `og_image` in `generateMetadata`, then mark up the body with
> `data-translate` keys (`about.headline`, `about.body`) and a
> `data-image-key="about.hero"` image. Wire `register_component` through
> the Neural Draft MCP server so the section is editable in the admin."_

Pair this with [`@neuraldraft/mcp`](/mcp/setup) installed in your editor —
...
- Build-time `generateStaticParams` is bounded by your rate limit. Use
  `page_size: 100` and paginate if you have more than a few hundred posts.
