...
const apiKey = import.meta.env.NEURALDRAFT_API_KEY ?? process.env.NEURALDRAFT_API_KEY;
if (!apiKey) throw new Error("Missing NEURALDRAFT_API_KEY");

export const nd = new NeuralDraftClient({ apiKey });

export function verifyNeuralDraftSignature(
  body: string,
  header: string,
  secret = import.meta.env.NEURALDRAFT_WEBHOOK_SECRET ?? "",
...
> _"Add a `/changelog` page that lists the last ten published blog posts
> tagged 'changelog'. Use the existing `nd` client from `src/lib/neural-draft.ts`
> and call `nd.blogPosts.list({ status: 'published', tag: 'changelog',
> page_size: 10, sort: '-published_at' })`. Render every text node with
> a `data-translate` key (`changelog.title`, `changelog.description`)
> so the customer can edit copy in the Neural Draft admin."_

With [`@neuraldraft/mcp`](/mcp/setup) installed, the AI will also call
